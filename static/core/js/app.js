...
async function getCsrfToken() {
  ...
  if (!response.ok) throw new Error('Could not prepare the contact form. Please refresh and try again.');

  token = getCookie('csrftoken');
  if (!token) throw new Error('Could not prepare the contact form. Please refresh and try again.');
  return token;
}
